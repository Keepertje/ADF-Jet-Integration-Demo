 ...
 */
    oj.ArrayTableDataSource = function(data, options) {
        // Initialize
        this.data = data || {};
        // This was put in to keep closure happy...
        if (
        ...
        if (options == null || options['idAttribute'] == null) {
            oj.Logger.info(
                oj.ArrayTableDataSource._LOGGER_MSG['_INFO_ARRAY_TABLE_DATASOURCE_IDATTR']
            );
        }

        oj.ArrayTableDataSource.superclass.constructor.call(
            this,
            data,
            options
        );
        ...
        if (data != null && data !== undefined) {
            ...
            this._data = data instanceof Array ? data : /** @type {Function} */
                data();
        ...
        if (
            options != null &&
                (options['startFetch'] == 'enabled' ||
                    options['startFetch'] == null) ||
                options == null
        ) {
    ...
    oj.ArrayTableDataSource.prototype.sort = function(criteria) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            self._rows['data'].sort(function(a, b) {
                return oj.ArrayTableDataSource._sortFunc(
                    a,
                    b,
                    comparator,
                    self
 ...
 */
    ...
    oj.ArrayTableDataSource.prototype._addToRowSet = function(
        m,
        index,
        options
    ) {
            ...
            if (row != null) {
                ...
                if (this._sorted == true && this._rows['data'].length > 0) {
                    ...
                    for (j = 0; j < this._rows['data'].length; j++) {
                        if (
                            oj.ArrayTableDataSource._sortFunc(
                                row,
                                this._rows['data'][j],
                                self._getComparator(),
                                self
    ...
    oj.ArrayTableDataSource.prototype._fetchInternal = function(options) {
        ...
        try {
            ...
            this._startIndex = options['startIndex'] == null
                ? this._startIndex
                : options['startIndex'];
            var endIndex = oj.ArrayTableDataSource._getEndIndex(
                this._rows,
                this._startIndex,
                pageSize
            );
 ...
 * @private
 */
    oj.ArrayTableDataSource.prototype._endFetch = function(
        options,
        result,
        error
    ) {
        if (error != null) {
            ...
            if (!options['silent']) {
                oj.TableDataSource.superclass.handleEvent.call(
                    this,
                    oj.TableDataSource.EventType['SYNC'],
                    result
                );
            }
        }
    ...
    oj.ArrayTableDataSource._getEndIndex = function(
        rows,
        startIndex,
        pageSize
    ) {
        ...
        if (pageSize > 0) {
            ...
            endIndex = endIndex > rows['data'].length - 1
                ? rows['data'].length - 1
                : endIndex;
        }
        ...
        if ($.isArray(idAttribute)) {
            ...
            for (i = 0; i < idAttribute.length; i++) {
                if (idAttribute[i] in row) {
                    id[i] = oj.ArrayTableDataSource._getKey(
                        row,
                        idAttribute[i]
                    );
                } else {
                    var errDetail = oj.Translations.applyParameters(
    ...
    oj.ArrayTableDataSource._sortFunc = function(a, b, comparator, self) {
        ...
        if ($.isFunction(comparator)) {
            ...
            if (comparator.length === 1) {
                ...
                var attrs1 = oj.StringUtils.isString(keyA)
                    ? keyA.split(',')
                    : [keyA];
                var attrs2 = oj.StringUtils.isString(keyB)
                    ? keyB.split(',')
                    : [keyB];
                for (i = 0; i < attrs1.length; i++) {
                    retVal = oj.ArrayTableDataSource._compareKeys(
                        attrs1[i],
                        attrs2[i],
                        direction
                    );
